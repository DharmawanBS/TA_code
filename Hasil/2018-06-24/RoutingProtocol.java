...
public class RoutingProtocol implements RouteInterface.ZRP_Route {
    ...
    private static final boolean direct_send = false;
    ...
    NodesList list2 = new NodesList();
    
    NodesList list_in = new NodesList();
    NodesList list_out = new NodesList();
    ...
    private void handleMessagePool(NetMessage msg) {
        ...
        NodeEntry next = findNextHop(mpdv.sinkLocation,false,null,mpdv);
        
        if (next == null) {
            System.out.println("Stop "+myNode.getID());
            return;
        }
    ...
    private NodeEntry findNextHop(NCS_Location2D loc,boolean intra,MessageDataValue msg,MessagePoolDataValue msg2) {
        ...
        NodeEntry nextHop = (intra) ? findNextHop_in(loc) : findNextHop_out(loc,msg2.sinkIP);
        ...
        if (myNode.neighboursList.getAsLinkedList().isEmpty()) {
            myNode.stop = true;
            return null;
        }
    ...
    private NodeEntry findNextHop_out(NCS_Location2D loc,NetAddress sink) {
        NodeEntry nextHop = null;
        
        double distance = myNode.getNCS_Location2D().distanceTo(loc);
        //if (myNode.getID() == 178) System.out.println(""+myNode.getID()+" "+distance);
        boolean use_sink = false;
        if (distance < TRESHOLD && myNode.neighboursList.contains(sink)) {
            use_sink = true;
        }
        ...
        if (list.size() == 0 || list2.size() == 0) {
            for(NodeEntry neighbourEntry: neighboursLinkedList) {
                ...
                double tmp = neighbourEntry.getNCS_Location2D().distanceTo(loc);
                if (use_sink && tmp < TRESHOLD) {
                    continue;
                }
                if (tmp < distance) {
        ...
        if (list_in.size() == 0 && list_out.size() == 0) {
            list_in = list;
            list_out = list2;
        }
        ...
        if (list_in.size() > 0) {
            nextHop = list_in.getAsLinkedList().get(0);
            list_in.remove(nextHop);
        ...
        if (nextHop == null)  {
            nextHop = list_out.getAsLinkedList().get(0);
            list_out.remove(nextHop);
    ...
    private void handleMessageDataValue(MessageDataValue msg) {
        ...
        if (CH_ip == myNode.getIP()) {
            ...
            //cari next hop;
            NodeEntry nextHop = findNextHop(CH_loc,true,msg,null);
    ...
    private NodeEntry getMyClusterHead(MessageDataValue msg) {
        ...
        if (direct_send && myNode.neighboursList.contains(detailQueryProcessed.get(msg.queryId).sinkIP)) {
            return myNode.neighboursList.get(detailQueryProcessed.get(msg.queryId).sinkIP);
        }
    ...
    public void dropNotify(Message msg, MacAddress nextHopMac, Reason reason) {
            ...
            if (!this.netQueueFULL) {
                ...
                if (remove) {
                    ...
                    for (int i=0;i<ZONE_COUNT;i++) {
                        zone.zone.get(i).zone_item.remove(nextHopMac.hashCode());
                    }
                    list.remove(nextHopMac.hashCode());
                    list2.remove(nextHopMac.hashCode());
                    list_in.remove(nextHopMac.hashCode());
                    list_out.remove(nextHopMac.hashCode());
                }
            }
        }
        ...
        JistAPI.sleepBlock(1000 * Constants.MILLI_SECOND);
